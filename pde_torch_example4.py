...
t_bc_ones = 5*np.ones((boundnum, 1))
...
for i in range(x.shape[0]):
    f[i, 0] =0
    Exact1[i, 0] = r/2+r/2*np.tanh(r/2*np.sqrt(1/2)*(x[i, 0]-((2-r)/(2))*(np.sqrt(2)/gamma(1+alpha))*t[i, 0]**alpha))
...
for epoch in range(iterations):
    ...
    t_bc_var = np.random.uniform(low=0, high=5, size=(boundnum, 1))
    x_bc_var = np.random.uniform(low=0, high=1, size=(boundnum, 1))
    u_bc_x0 = r/2+r/2*np.tanh(delta*r/2*np.sqrt(1/(1+delta))*(-((1+delta-r)/(1+delta))*(np.sqrt(1+delta)/gamma(1+alpha))*(t_bc_var**alpha)))
    u_bc_x1 = r/2+r/2*np.tanh(delta*r/2*np.sqrt(1/(1+delta))*(1-((1+delta-r)/(1+delta))*(np.sqrt(1+delta)/gamma(1+alpha))*(t_bc_var**alpha)))
    u_ic_t0 = r/2+r/2*np.tanh(delta*r/2*np.sqrt(1/(1+delta))*x_bc_var)
    u_ic_t1 = r/2+r/2*np.tanh(delta*r/2*np.sqrt(1/(1+delta))*(x_bc_var-((1+delta-r)/(1+delta))*(np.sqrt(1+delta)/gamma(1+alpha))*5**alpha))


    # 将数据转化为torch可用的
    pt_x_bc_var = Variable(torch.from_numpy(x_bc_var).float(), requires_grad=False)
    pt_t_bc_var = Variable(torch.from_numpy(t_bc_var).float(), requires_grad=False)


    pt_u_bc_x0 = Variable(torch.from_numpy(u_bc_x0).float(), requires_grad=False)
    pt_u_bc_x1 = Variable(torch.from_numpy(u_bc_x1).float(), requires_grad=False)
    ...
    net_bc_inr = net(torch.cat([pt_t_bc_var, pt_x_zero], 1))  # 0=u(t,1) 公式（3)
    net_bc_inl = net(torch.cat([pt_t_bc_var, pt_x_one], 1))  # 0=u(t,0) 公式（4）

    mse_u_3 = mse_cost_function1(net_bc_inr, pt_u_bc_x0)  # e = 0-u(t,10) 公式(3)
    mse_u_4 = mse_cost_function1(net_bc_inl, pt_u_bc_x1)  # e = 0-u(t,-10) 公式（4）
    ...
    w5 = mse_u_5 / minmse
    # 将误差(损失)累加起来
    #loss = w1*mse_f_1+(w2*mse_u_2+w3*mse_u_3+w4*mse_u_4+w5*mse_u_5)
...
for i in range(x.shape[0]):
    u_realfla[i, 0]= (r/2+r/2*np.tanh(delta*r/2*np.sqrt(1/(1+delta))*(x[i, 0]-((1+delta-r)/(1+delta))*(np.sqrt(1+delta)/gamma(1+alpha))*t[i, 0]**alpha)))**(1/delta)
